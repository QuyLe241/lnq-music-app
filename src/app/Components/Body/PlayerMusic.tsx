...
const PlayerMusic: React.FC = () => {
  ...
  const handleNextSong = () => {
    if (currentSong) {
      const currentIndex = dataSong.findIndex(
        (song) => song.id === currentSong.id
      );
      if (currentIndex < dataSong.length - 1) {
        setCurrentSong(dataSong[currentIndex + 1]);
        setIsPlaying(true);
      }
    }
  };
  ...
  const isFirstSong = currentSong
    ? dataSong.findIndex((song) => song.id === currentSong.id) === 0
    : false;

  const isLastSong = currentSong
    ? dataSong.findIndex((song) => song.id === currentSong.id) ===
      dataSong.length - 1
    : false;
